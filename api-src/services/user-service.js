...
class UserService {
    ...
    async create(email, hashPassword, salt) { 
        try{       
            ...
            if (user) { 
                throw new ServiceError(409, Status.USERS_WRONG_PASSWORD, "해당 이메일로 가입된 유저가 이미 있습니다.", {email : email});
            }
            else { 
            ...
            console.log(err);
            throw new ServiceError(err.httpStatus || 400, err.status || 1, err.message || 'users database error');
        }    
    }
}
